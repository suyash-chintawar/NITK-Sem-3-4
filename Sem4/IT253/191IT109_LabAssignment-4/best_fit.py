#Best-Fit memory allocation algorithm using fixed partitioning
...
#for each process find the smallest free block which has size greater than size of process
for i in range(num_processes):
	...
	for j in range(num_blocks):
		if occupied[j]==0 and block_sizes[j]>=process_sizes[i]:
			if idx==-1:
				idx=j
			elif block_sizes[idx]>block_sizes[j]:
				idx=j
	if idx!=-1:
		allocated[i]=idx
		occupied[idx]=1

...
while completed!=n: #loop until all processes are completed
	for i in range(n): #push already arrived processes which are not yet completed in queue_data[]
		if process_data_copy[i][1]<=time and process_data_copy[i][3]==0:
			queue_data.append(process_data_copy[i])
	queue_data.sort(key = (lambda x: x[2])) #sort according to remaining time
	if(len(queue_data)<1): #if queue is empty
		time+=1
		continue
	curr=queue_data.pop(0) #pop first element from queue
	time+=1 #execute process for one unit time
	for i in range(n):
		if(process_data_copy[i][0]==curr[0]):
			process_data_copy[i][2]-=1
			if process_data_copy[i][2]==0: #if process completed update parameters
